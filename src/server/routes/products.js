...
router.route("/add").post((req, res) => {
    ...
    const newProduct = new Product({ title,category,price,discount_price,slug,quantity,sku,weight});
...
router.route('delete/:id/').delete(auth,(req,auth, res) => {
    Product.findByIdAndDelete(req.params.id)
        .then(() => res.json('Exercise deleted.'))
        .catch(err => res.status(400).json('Error: ' + err));
});

router.route('/update/:id').put(auth,(req,auth, res) => {
    Product.findById(req.params.id)
        .then(product => {
